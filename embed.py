...
def generate_prompts(df: pd.DataFrame) -> List[str]:
    ...
    try:
        ...
        print("Warning: 'criteria_embed.md' not found. Using an empty prefix for embeddings.")
...
def main():
    parser = argparse.ArgumentParser(description="Generate LLM embeddings for papers from a CSV file using OpenRouter.")
    parser.add_argument("csv_file", type=str, help="Path to the input CSV file.")
    parser.add_argument("-n", type=str, default="all", help="Number of papers to process (integer) or 'all' for all papers.")
    ...
    if (args.p is not None) and (PROBABILITY_COLUMN not in original_df.columns):
        sys.exit(f"Error: DataFrame must contain a '{PROBABILITY_COLUMN}' column when using -p.")
