...
def create_structured_response_model(classification_classes: List[str]) -> type(BaseModel):
...
async def process_prompts_for_model(
    ...
    async with aiohttp.ClientSession() as session:
        api_call_tasks = [
            call_openrouter_async(prompt, model, api_key, session, semaphore, structured_schema=schema)
...
def parse_results(
    ...
    if len(tasks) != len(results):
        print("Warning: Mismatch between number of tasks and results. Skipping remaining papers.")
    ...
    if len(results) != expected_result_count:
        print(f"Error: Expected {expected_result_count} results, got {len(results)}. Results parsing may be inaccurate.")
    ...
    for i in range(n_papers):
        ...
        if len(paper_results) < n_classifications_per_paper:
            print(f"Warning: Only {len(paper_results)} results found for paper {i}. Filling remaining with 'N/A'.")
            
        paper_results_dict = {}
        
        for res_idx, classification in enumerate(classification_objects):
            classification_raw_name = classification["name"]
            classification_name = classification_raw_name.replace(" ", "_").replace("-", "_")
            ...
            if result and 'choices' in result and result['choices']:
                try:
                    ...
                    if chosen_class not in class_options:
                        print(f"Warning: LLM returned invalid class '{chosen_class}' for paper index {i}, classification {classification_name}. Setting to 'other'.")
                        chosen_class = 'other'
