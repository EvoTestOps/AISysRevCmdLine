...
def save_response_to_md(response: dict, csv_path: Path, model: str, n_papers: int) -> None:
    """Save only the classification results to a Markdown file, avoiding overwrites."""
    sanitized_model = "".join(c if c.isalnum() or c in ["-", "_"] else "_" for c in model)
    base_output_filename = csv_path.with_stem(f"{csv_path.stem}_n_{n_papers}_LLM_class_gen_{sanitized_model}").with_suffix(".md")

    # Check if file exists and find a non-conflicting name
    output_filename = base_output_filename
    counter = 1
    while output_filename.exists():
        output_filename = csv_path.with_stem(f"{csv_path.stem}_n_{n_papers}_LLM_class_gen_{sanitized_model}({counter})").with_suffix(".md")
        counter += 1
